...
class mobile {
    ...
    public static function mobile_forumng_init(array $args) : array {
        ...
        return [
                'templates' => [],
                'javascript' =>
                        file_get_contents($CFG->dirroot . '/mod/forumng/appjs/' . $foldername . 'mobile_init.js'),
                'otherdata' => '',
                'files' => []
        ];
    }
    ...
    public static function posts_view(array $args) : array {
        ...
        return [
            ...
            'otherdata' => [
                ...
                'candeletediscussion' => $discussion->can_delete($whynot),
                'isdeleteddiscussion' => $discussion->is_deleted(),
                'textdelete' => get_string($discussion->is_deleted() ? 'undeletediscussion' : 'deletediscussion',
                        'forumngfeature_delete'),
    ...
    public static function mobile_get_folder_name($args): string {
        if (!isset($args->appversioncode)) {
            return self::IONIC7_FOLDER;
        }

        return match (true) {
            $args->appversioncode < 3950 => self::IONIC3_FOLDER,
            $args->appversioncode < 44000 => self::IONIC5_FOLDER,
            default => self::IONIC7_FOLDER,
        };
    }
}
